...
final class LineNumberTextView: NSTextView {
    ...
    var lineNumberColor: NSColor = NSColor.gray.withAlphaComponent(0.4) {
    ...
    var findMatchColor: NSColor = NSColor.systemYellow.withAlphaComponent(0.45)
    /// Background color for the current find match.
    var findCurrentMatchColor: NSColor = NSColor.systemOrange.withAlphaComponent(0.85)
    ...
    @discardableResult
    func applyFindHighlights(query: String, options: FindOptions, currentIndex: Int, active: Bool) -> (count: Int, regexInvalid: Bool) {
        clearFindHighlights()
    ...
    func clearFindHighlights() {
        guard let layoutManager else { findHighlightRanges = []; return }
