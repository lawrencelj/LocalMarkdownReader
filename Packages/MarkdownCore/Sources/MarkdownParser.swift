...
public actor MarkdownParser {
    ...
    public struct Configuration: Sendable {
        ...
        public static let secure = Configuration(enableSecurityValidation: true, allowUnsafeHTML: false)
    ...
    public func parseDocument(content: String, reference: DocumentReference) async throws -> DocumentModel {
        return try await performanceMonitor.trackOperation("parse_document") {
            ...
            } else if Int64(content.utf8.count) > Self.maxDocumentSize {
                throw DocumentError.fileTooLarge(maxSize: Self.maxDocumentSize)
            }

            let attributedContent: AttributedString
            if format == .markdown || format == .plainText {
                attributedContent = try await parseToAttributedString(content)
            } else {
            ...
            if format == .latex {
                metadata = LaTeXDocumentProcessor.metadata(from: content, reference: reference)
            } else {
                metadata = try await extractMetadata(from: content, reference: reference)
            }

            let outline: [HeadingItem]
            if format == .latex {
                outline = LaTeXDocumentProcessor.outline(from: content)
            } else if format.isStructuredText {
                outline = StructuredOutlineExtractor.extract(from: content, format: format)
            } else {
                outline = try await extractOutline(from: content)
            }
    ...
    public nonisolated func parseToAttributedString(_ content: String) throws -> AttributedString {
