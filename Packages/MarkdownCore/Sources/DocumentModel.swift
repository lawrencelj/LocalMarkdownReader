...
public enum DocumentFormat: String, Sendable, Codable {
    ...
    public var isStructuredText: Bool {
        self == .json || self == .xml
    }
}
...
public struct DocumentModel: Sendable, Codable, Identifiable, Hashable {
    ...
    public var format: DocumentFormat {
        DocumentFormat(url: reference.url)
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        // Re-parse attributed content (not serializable).
    ...
    /// Estimated reading time from metadata
...
public struct DocumentMetadata: Sendable, Codable, Hashable {
    ...
    /// Convenience accessor for encoding
...
extension DocumentModel {
    ...
    public static var preview: DocumentModel {
        ...
        let parser = MarkdownParser()
        let attributedContent = (try? parser.parseToAttributedString(content)) ?? AttributedString(content)
