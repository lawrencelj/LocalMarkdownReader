...
final class MarkdownCoreTests: XCTestCase {
    ...
    func testContentValidation() async throws {
        ...
        do {
            _ = try await documentService.parseMarkdown(validContent)
        } catch {
            XCTFail("Valid content should not throw: \(error)")
        }
    ...
    func testLaTeXDocumentParsingExtractsStructureAndMetadata() async throws {
        let content = #"""
        \documentclass{article}
        \title{A \textbf{Useful} Paper}
        \author{Ada Lovelace}
        \begin{document}
        \maketitle

        % \section{Hidden Comment}
        \section{Overview}
        ...
        """#
        let reference = DocumentReference(
            url: URL(fileURLWithPath: "/tmp/useful-paper.tex"),
        ...
        XCTAssertEqual(document.title, "A Useful Paper")
        XCTAssertEqual(document.outline.map(\.title), ["Overview", "Implementation"])
        XCTAssertEqual(document.outline.map(\.level), [1, 2])
        XCTAssertTrue(document.metadata.hasImages)
        XCTAssertTrue(document.metadata.hasTables)
        XCTAssertTrue(document.metadata.hasCodeBlocks)
        XCTAssertEqual(document.metadata.languageHints, ["swift"])
        XCTAssertGreaterThan(document.metadata.wordCount, 5)
    }

    func testLaTeXAndTexExtensionsAreRecognized() {
        XCTAssertEqual(
            DocumentFormat(url: URL(fileURLWithPath: "/tmp/document.tex")),
            .latex
        )
        XCTAssertEqual(
            DocumentFormat(url: URL(fileURLWithPath: "/tmp/document.latex")),
            .latex
        )
    }
}
